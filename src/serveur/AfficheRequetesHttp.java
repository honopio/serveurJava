 ...
 * preconditions : the server must be started with a valid port number (positive integer) and an existing directory path
 * 
 * if the requested resource is a directory, an html page listing its content is sent in the HTTP response
 * if it's a file, its content is sent
 * otherwise, a 404 error response is sent
 */
...
public class AfficheRequetesHttp implements Runnable {
	...
	public static void main(String[] args) throws NoSuchFileException {
		...
		try (ServerSocket s = new ServerSocket(portNumber);) {
			...
			System.err.println("IO error with the ServerSocket : ");
			e.printStackTrace();	
		}
	}
	...
	public void run() {
		...
		if (sc.hasNextLine()) { //sans ce if, continue d'essayer de lire donc plante au bout d'un moment
			...
			if (firstline.substring(0, 3).equals("GET")) {
				...
				// "GET /" renvoie le contenu de DossierRoot
			    if (requestedFile.equals("/")) {
			        requestedFilePath = Paths.get(DossierRoot);
			    } else { //sinon on concatene le chemin de DossierRoot avec le chemin du requested file
				...
				if (Files.isDirectory(requestedFilePath)) {
					...
					if (derniersSlash == requestedFile.indexOf('/')) { //s'il y a un seul slash dans le chemin, le Parent Directory est DossierRoot
				...
				//SINON (envoyer erreur 404)
					...
					pw.println("The requested resource does not exist");
					pw.flush(); // il faut vider le buffer pour que le contenu soit envoyé.
				}
